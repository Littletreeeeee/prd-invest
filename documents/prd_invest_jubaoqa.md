...
（1）稿件入库、打标：聚宝传入文件数据字典见[功能详情-问答稿件入库](#21-问答稿件入库)，入库打标规则见[功能详情-入库打标规则](#22-入库打标规则)；入库的问答稿件可在栏目稿库中筛选查看，并进入编辑页面。  

（2）编辑稿件内容、标签：可编辑问题标题、问题描述、回答正文，见[功能详情-问答编辑页面](#23-问答编辑页面)；稿件标签可编辑产品标签、业务标签，见[功能详情-标签编辑](#24-标签编辑)。   

（3）保存稿件：可将未处理完的问答文件保存至草稿箱，见[功能详情-稿件操作](#25-稿件操作)。  

（4）发布稿件：稿件发布后将稿件文件推送给聚宝，见[功能详情-稿件文件推送](#26-稿件文件推送)；同时可在资讯审核-栏目中管理对已发布的问答稿件进行修改和下线，目前暂未联调稿件的更新和下线，待补充。
...
|question_editable|问题是否可修改|待定|
|answer_editable|回答是否可修改|待定|
...
2. 业务标签的打标：根据 **问题标题、问题描述、回答正文** 内容与[附录1：业务标签列表](#附录1业务标签列表)中 **二、三级** 标签的匹配情况（完全匹配），匹配对应的二级、三级标签。业务标签的匹配考虑三级和二级的关联关系：如匹配到了某三级标签，未匹配到其对应的二级标签，则同时增加此三级标签及其对应的二级标签；但是若匹配到了某二级标签，未匹配到其下的三级标签，则只增加此二级标签。
...
1) 问题标题输入，最多1000字符，对应500中文字；问题描述输入，最多4000字符，对应2000中文字；回答正文输入，最多10000字符，对应5000中文字。第一期除了`<p>`标签用于分段以外，问答内容不包含其他html标签，所以原编辑器中的加粗、超链接、插入图片功能可暂时禁用。
...
3）标签在界面上分为业务标签和产品标签。
  ...
  * 产品标签：产品标签包括三级。一级在界面上显示为 **产品标签** ，包括沪深、港股、美股、基金四个产品类别；二级在界面上显示为 **行业板块、地域板块、概念板块、基金类型** ，其中板块列表是同步的聚源数据，基金类型目前没有除了，需增加[附录2：产品标签列表](#附录2产品标签列表)中基金下的二级标签。产品标签层级间的勾选无关联逻辑。
...
### 2.5 稿件操作
针对稿件的操作包括：发布、保存、修改、下线、版本记录、检测重复。其中发布为第一期必做功能；修改、下线待定，视聚宝对接情况；保存、版本记录、检测重复为选作功能。

* 发布：在编辑页面点击发布按钮，则稿件进入资讯审核-栏目列表中，可用过文章类别进行筛选。同时向聚宝发送问答稿件文件。

* 保存：被保存的稿件存放在草稿箱中，且类型为“问答”。

* 版本记录：在编辑页面点击“版本记录”，或者稿件预览页面版本记录部分，可查看稿件的修改记录即修改内容。

* 检测重复：检测与其他已发布稿件、问答的重复情况。

### 2.6 稿件文件推送

> 注：业务标签和keywords的对应，暂不考虑，第一期keywords怎么进怎么出不进行编辑。二三级业务标签均通过tags传输。
...
|question_editable|无输入项|待定|
|answer_editable|无输入项|待定|
