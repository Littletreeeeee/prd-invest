...
其中 title、summary、content 按默认流程处理，会和栏目稿件一样做格式清洗。tags、keywords 目前按默认流程处理不会修改。以后涉及到自动处理这两个字段的时候需要注意。

不影响正常生产的前提下，目前后台处理能力可接受的自动回传类型稿件传入速度，为“”每半小时，发送4分钟，约3000条”。
